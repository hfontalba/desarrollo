...
" ".$merchant_info['post_code'];
	    ...
	    <div class="receipt-main-wrap">		   
	       ...
	       <div class="input-block">
            ...
            <div class="input-block">
	         <div class="label"><?php echo Yii::t("default","Identification Card")?> :</div>
	         <div class="value"><?php echo $data['id_card']?></div>
	         <div class="clear"></div>
	       </div>
	       ...
	       <?php if ($data['trans_type']=="delivery"):?>
		       ...
		       <div class="input-block">
		         ...
		         <div class="value">
		         	<?php
		         	 $d_created = date('M d,Y G:i:s',strtotime($data['date_created']));
		         	 $d_date = date('M d,Y G:i:s',strtotime($data['delivery_date']));
		         	 $d_time =  date('g:i:A', strtotime($data['delivery_time']));
		         	 $partials = explode(':',$d_time);
		         	 if($partials[2] == 'PM'){
		         	     $partials[0] = $partials[0]+12;
		         	 }
		         	 $d_date = date('M d,Y G:i:s',strtotime('+'.$partials[0].' hours +'.$partials[1].' minutes',strtotime($data['delivery_date'])));
		         	 //$d_time = date('g:iA', strtotime($data['delivery_time']));
		         	 $diff = strtotime($d_date) - strtotime($d_created);
					 ...
					 $days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24)); //$days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
					 $hours = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24 - $days*60*60*24) / (60*60));//$hours = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24 - $days*60*60*24) / (60*60));
					 $minutes = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24 - $days*60*60*24 - $hours*60*60)/ 60);//$minutes = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24 - $days*60*60*24 - $hours*60*60)/ 60);
					 //$seconds = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24 - $days*60*60*24 - $hours*60*60 - $minutes*60));
                    
		         	 $date = prettyDate($data['delivery_date']);
				     $date=Yii::app()->functions->translateDate($date);
			         if ($diff > 0 && ($minutes > 45 || $hours >= 1 || $days > 0)){
		       ...
		       <div class="input-block">
		         ...
		         <?php //echo Yii::app()->functions->timeFormat($data['delivery_time'],true)
		            $dtime = Yii::app()->functions->timeFormat($data['delivery_time'],true);
		            if ($diff > 0 && ($minutes > 45 || $hours >= 1 || $days > 0)){
						echo '<span style="color: red;">'.$dtime.'</span>';
			         } else echo $dtime;
		         ?>
		         </div>
		         <div class="clear"></div>
