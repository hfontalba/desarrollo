...
<?php echo CHtml::hiddenField('action','FoodItemListUpdate')?>
		...
		if ( is_array($price) && count($price)>=1){
			foreach ($price as $key_price=>$val_price) {    	    		
				if (array_key_exists($key_price,(array)$size_list)){
					$price_list.= ucwords($size_list[$key_price])." ".getCurrencyCode();
					
					$price_list .= '<input type="number" value="'.$val_price.'" min="0" step="0.01" id="price" name="price['.$row['item_id'].']['.$key_price.']" /><br/>';
