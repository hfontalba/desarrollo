...
class AddonMobileApp
{
	...
	{
		$cod=self::t("Transferencias bancarias");
	...
	public static function merchantInformation($merchant_id='')
	{
		...
		if ($merchantinfo=Yii::app()->functions->getMerchant($merchant_id)){
			...
			$minimum_order_raw=$minimum_order;
 			...
 			$delivery_est=getOption($merchant_id,'merchant_delivery_estimation');
 			$data['delivery_est']=$delivery_est;
		...
		if (is_array($cart) && count($cart)>=1){
			...
			foreach ($cart as $val) {
	        	...
	        	if($food['packaging_fee']!=0 || $food['packaging_fee']!=''){
	        		if($food['packaging_incremental'] == 1) {
	        			$merchant_packaging_charge2 += $food['packaging_fee']*$item_total;		
	        		}else{
	        			$merchant_packaging_charge2 += $food['packaging_fee'];
	        		}
	        	}
		    ...
		    if ($search_mode!="postcode"){
			    ...
			    if ( $data['transaction_type']=="delivery" ){		
			    	...
			    	/* PRUEBA CART */
			    	if( $data['google_lat'] != "" && $data['google_lng'] != "" ){
			    	    $cli_lat = $data['google_lat'];
    			        $cli_long = $data['google_lng'];
			    	}else{
			    	    if($data['formatted_address']!=''){
        			        $cli_geo=Yii::app()->functions->geodecodeAddress($data['formatted_address']);    
        			    }else{
        			        $formated_adr = $data['street'].'%2C+'.$data['city'].'%2C+'.$data['zipcode'].'%2C+'.$data['state'];
           ...
           /*get the tax*/
	        ...
	        if ( $merchant_tax>0){
	        	...
	        	if ( $merchant_tax_charges==2){
	        		...
	        		$tax_calc=($taxable_total+$delivery_charges+$merchant_packaging_charge+$merchant_packaging_charge2+($taxable_total*0.05))*($merchant_tax/100);
	        	} else {
	        		// TAX - PACKAGE CHARGE
			...
			if ($merchant_packaging_charge>0 || $merchant_packaging_charge2>0){
				$cart_final_content['packaging']=array(
				  'amount'=>$merchant_packaging_charge+$merchant_packaging_charge2,
				  'amount_pretty'=>AddonMobileApp::prettyPrice($merchant_packaging_charge+$merchant_packaging_charge2)
				);					
			}
			...
			$grand_total=$subtotal+$delivery_charges+$merchant_packaging_charge+$merchant_packaging_charge2+$tax+$tips_amount+$card_fee;
					...
					switch ($val) {
						case "cod":			
						    if (Yii::app()->functions->isMerchantCommission($mtid)){
						    	$merchant_payment_list[]=array(
								  'icon'=>'fa-exchange',
								  'value'=>$val,
								  'label'=>self::t("Transferencias bancarias")
								);
						    	continue;
						    }
						...
						case "pyr":	
						   ...
						   if (Yii::app()->functions->isMerchantCommission($mtid)){
							    ...
							    'icon'=>'ion-card',
							    'value'=>$val,
							    'label'=>self::t("Mis Pagos Mercantil")
							   );
						   	   continue;
						   }
   ...
   public static function getMerchantInfo($merchant_id='')
    ...
    public static function getAvatar($client_id='',$res='')
    {
    	...
    	} else return Yii::app()->getBaseUrl(true)."/assets/images/avatar.jpg?1";    	
    }
    ...
    public static function displayCashAvailable($merchant_id='', $services='')
    {
		...
		if (array_key_exists('cod',(array)$new_payment_list)){					
			switch ($services) {
				...
				case 4:	
				case 5:	
				    $payment_accepted[]=AddonMobileApp::t("Transferencias bancarias");			
					break;
				case 3:
   ...
   public static function saveGoogleAvatar($link='',$user_id='')
	...
	/* 1.0.11 */
	public static function getCategoryCountWithRestriction($merchant_id='', $days='')
	{
		$and='';
		$days=strtolower($days);  
    ...
    /* 1.0.11 */
	public static function getClientAge($client_id='')
	{
		$DbExt=new DbExt;
		$stmt="
		SELECT IFNULL(TIMESTAMPDIFF(YEAR,birthdate,CURDATE()),0) AS age
	    FROM
		{{client}}
		WHERE 
		client_id=".self::q($client_id);					
