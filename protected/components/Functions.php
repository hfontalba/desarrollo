...
{	
	...
	public function getNeedInventory($merchant_id='')
	{
		$DbExt=new DbExt;
		$stmt="SELECT a.need_inventory
				From {{merchant}} a
				WHERE a.merchant_id=".FunctionsV3::q($merchant_id)."";
		if ($res=$DbExt->rst($stmt)) {
			return $res[0];
		}
    ...
    public function jsLanguageAdmin()
    {
    	...
    	return array(
    	  ...
    	  "trans_4"=>Yii::t("default","¿Estás seguro?"),
    	  "trans_5"=>Yii::t("default","Sorry but Minimum order is"),
    	  "trans_6"=>Yii::t("default","Seleciona un método de pago"),
    	  ...
    	  'trans_52'=>t("Por favor valida el Captcha"),
    ...
    public function getFoodItem3($item_id='')
    {
    	$merchant_id=$this->getMerchantID();
    	$DbExt=new DbExt;
	    $stmt="SELECT item_id, price FROM
			{{item}}
			WHERE
			item_id=".FunctionsV3::q($item_id)."
			AND
    ...
    public function getFoodItem4($item_name='')
    {
    	$merchant_id=$this->getMerchantID();
    	$DbExt=new DbExt;
	    $stmt="SELECT item_id, item_cant FROM
			{{item}}
			WHERE
			item_id=".FunctionsV3::q($item_id)."
			AND
	...
	public function getFoodItemLists2($merchant_id='')
	{
		$where='';
		if (is_numeric($merchant_id)){
			$where=" WHERE merchant_id=".$this->q($merchant_id)."";
		...
		$data_feed='';
		$stmt="
		SELECT item_id, item_name, item_description, status, category, price, item_cant, packaging_fee, date_created, date_modified FROM
		{{item}}	
		$where	
		ORDER BY sequence ASC
		";				
			          ...
			          $htm.='<div class="manage">';
			            $htm.='<div class="c">';
			             ...
			             $htm.='<a href="javascript:;" class="edit_item" data-row="'.$key.'" rel="'.$val['item_id'].'" >
			              ...
			              $htm.='<a href="javascript:;" style="margin-left:-20px !important;background:red;" class="delete_item" data-row="'.$key.'" rel="'.$val['item_id'].'" >
...
'sub_item_name',$addon_name_trans).'</div>';
    		    ...
    		    if ( !empty($tax)){
    				...
    				if ($subtotal_non>=0.0001){    					
    					...
    					if($temp_subtotal>=0.0001){
    					   ...
    					   $taxable_subtotal=($temp_subtotal+$temp_delivery_charges)*$tax;
    					}
    				} else {
    				    // TAX - PACKAGE CHARGE
                        // $taxable_subtotal=($temp_subtotal+$temp_delivery_charges+$merchant_packaging_charge)*$tax;
    					$taxable_subtotal=($subtotal+$temp_delivery_charges)*$tax;    
    				}
    			...
    			//$tax_total=($delivery_charges+$cargos+$subtotal+$merchant_packaging_charge)*0.16;
    			//$total=$delivery_charges+$cargos+$subtotal+$merchant_packaging_charge+$tax_total;
		        $total=$subtotal+$taxable_subtotal+$delivery_charges+$merchant_packaging_charge+$cargos; //posible eureka
		        ...
		        $htm.='<div class="summary-wrap">';
		         ...
		         if (isset($data['tip_enabled'])){		         	
		         	if ($data['tip_enabled']==2 && $data['tip_percent']>=0.0001){
		         		...
		         		$data['cart_tip_percentage']=0.16*100;
		       ...
		       if($has_discount){
                ...
                /* APP CHARGES */
                $htm.="<div class=\"row app_charges_wrap\">";
                $htm.="<div class=\"col-md-6 col-xs-6  text-right\">Cargos por uso del app</div>";
                $htm.="<div class=\"col-md-6 col-xs-6  text-right app_charges\">".
                displayPrice(baseCurrency(),prettyFormat($cargos,$mid))."</div>";
                $htm.="</div>";
    		...
    		} else $this->msg=Yii::t("default","¡No se ha añadido nada al carrito!");
	    } else $this->msg=Yii::t("default","¡No se ha añadido nada al carrito!");
	}
