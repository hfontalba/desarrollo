	...
	class AjaxAdmin extends DbExt
	{
		...
		{			
			...
			$params=array(
			  ...
			  'age_restriction'=>isset($this->data['age_restriction'])?$this->data['age_restriction']:0,
	    ...
	    /*FOOD ITEM MASIVE UPDATE*/

	    public function FoodItemListUpdate()
	    {	  
	        $mtid=Yii::app()->functions->getMerchantID();
	        $index = '';
	        $p_price = [];
	        ...
	        foreach($this->data['price'] as $key=>$val){
	            ...
	            $act_prod = Yii::app()->functions->getFoodItem3($prod);
	            $act_prices = json_decode($act_prod['price'], true);
	            
	            $flag = true; //Bandera que indica que los precios son validos
	            
	            foreach($val as $key2=>$val2){
	                if($val2 > 0 && $val2 != ''){
	                    $p_price[$key2]=$val2;
	                }else{
	                    $p_price[$key2]=$act_prices[$key2];
	                }
	            }
                ...
                $params=array(
                    ...
                    'ip_address'=>$_SERVER['REMOTE_ADDR'],
                    'merchant_id'=>$mtid,
                    'price'=>json_encode($p_price)
                );
                
                $res = $command->update('{{item}}' , $params , 'item_id=:item_id' , array(':item_id'=>$prod));
	        ...
	        if($result){
	            $this->code=1;
	            $this->msg=Yii::t("default","Items Updated.");    
	        }else{
...
$resto_info.="<p><span class=\"uk-text-bold\">".Yii::t("default","Delivery Est").": </span> ".$delivery_est."</p>";
    ...
    if (is_numeric($mt_delivery_miles)){    	
	    ...
	    {	    		   
	    	...
	    	$this->msg=t("Item añadido al carrito");	    	
	    	if (isset($this->data['item_id'])){	    		
	    		$item=$this->data;
	    		//dump($item); die();
	    ...
	    {	
	    	...
	    	if ( Yii::app()->functions->clientAutoLogin($this->data['username'],
	    		...
	    		$this->code=1;
	    		$this->msg=Yii::t("default","¡Bienvenido!");
	    	} else {
...
//dump($params); die();
	    ...
	    {	    		    
	    	...
	    	if ( $res=$DbExt->rst($stmt)){
	    		...
	    		foreach ($res as $val) {	    			
	    			$new='';
	    			//$action="<a data-id=\"".$val['order_id']."\" class=\"edit-order\" href=\"javascript:\">".Yii::t("default","Edit")."</a>";
	    			// quitar editar del panel de control
	    			$action="";
	    			$action.="<a data-id=\"".$val['order_id']."\" class=\"view-receipt\" href=\"javascript:\">".Yii::t("default","View")."</a>";
	    			...
	    			$feed_data['aaData'][]=array(
	    			  ...
	    			  FunctionsV3::prettyPaymentType('payment_order',$val['payment_type'],$val['order_id'],$val['trans_type']),
	    			  prettyFormat($val['sub_total'],$merchant_id),
                      PrettyFormat(($val['total_w_tax'])-($val['sub_total']*0.05)-($val['delivery_charge']),$merchant_id),
	    			  "<span class=\"tag ".$val['status']."\">".t($val['status'])."</span>",
	    			  t($val['request_from']),
	    			  $date,
	    			  $action
        ...
        //                 exit; 
	    			// 	break;
	    			//csv para exportar informe
	    			case 'rptAdminSalesMerchant':

/******************* Reporte de Ventas Diarias por Establecimientos **************************/
	    			/*** CODIGO MODIFICADO POR:
						 @author: Henyr Fontalba
						 Objetivo: Reestructurar y modificar el reporte
						 de informe de ventas diarias
						***/

	    				/*$head='<div class="uk -form -row" style="margin-bottom: 20px; margin-left: 20px">';
	    				$head.='<table width="25%" align="left" style="padding-left: 40%">
									<tr align="left" valign="center" style="margin-top: 2%; margin-bottom: 2%; height: 20px;">
										<td colspan="2">
											<img src="http://www.asiderapido.com/assets/images/logo-desktop.png">
										</td>
									</tr>';
						$head.='<tr style="height: 50px;"></tr>';
						$head.='<tr style="height: 50px;"></tr>';

						$head.='<tr align="left" valign="center" style="margin-top: 20%; border:solid 1px; border-color: solid black; width: 2px;">
									<td align="left"  colspan="2" valign="center" style="text-align: center; font-size: 18; font-style: italic; font-style: bold; max-width: 50%; border-color: solid white;"><b>'.Yii::t("default", "INFORME DE VENTAS").'</b></td>
	                    ...
	                    if ($res=$db_ext->rst($stmt)){                
	                            foreach ($res as $val) {                                
	                                $date=prettyDate($val['date_created'],true);
	                                $date=Yii::app()->functions->translateDate($date);
	                                $name = stripslashes($val['restaurant_name']);
	                                $item=FunctionsV3::translateFoodItemByOrderId($val['order_id']);
	                                $subtotal = ($val['sub_total'])+($val['packaging']);
	                                $deduccion = (($val['sub_total'])*($val['percent_commision']/100));
	                                $total_transf = (($val['sub_total'])+($val['packaging'])) - (($val['sub_total'])*($val['percent_commision']/100));
	                                
	                                $data[]=array('<tr align="center" valign="center" width="90%" style="border:solid 1px; border-radius:"1px" border-color:solid black; margin-top: 4px;><td align="center" valign="center" style="text-align: center;">'.$val['order_id'].'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.$val['qty_item'].'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.$item.'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.strtoupper(Yii::t("default",$val['payment_type'])).'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">None</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">None</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.$subtotal.'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.$deduccion.'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.$total_transf.'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">None</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.ucwords($val['status']).'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.ucwords($val['request_from']).'</td></tr>');
	                            }                        
	                        }                

	                        $head.='<tr class="table table-bordered" align="left" valign="center" style="background-color: #58ACFA; border: solid 1px; border-radius: 2px; border-color: balck;">
									<td align="center" valign="center" style="text-align: center;"><b>'.Yii::t("default", "Nombre del Establecimiento").'</b></td>
									<td align="center" valign="center" style="text-align: center;">'.$name.'</td>

								</tr>';
							$head.='<tr class="table table-bordered" align="left" valign="center" style="margin-top: 40px; border: solid 1px; border-radius: 2px; background-color: #58ACFA; border-color: balck;">
										<td align="center" valign="center" style="text-align: center; color: black;"><b>'.Yii::t("default", "Fecha de Cierre").'</b></td>
										<td align="center" valign="center" style="text-align: center; color: black;"><b>'.$date.'</b></td>
									</tr>';

							$head.='	</table>
									 </div>';		

							$head.='<div class="container" style="max-width: 90%; margin-left: 5%" align="center">
										<table class="table table-bordered" width="90%" align="left">';
							$head.='<tr style="height: 35px;"></tr>';
	                        $header=array($head,
	                        '<tr align="center" valign="center" style="argin-top: 40px; border: solid 1px; border-radius: 2px; border-color: balck;"><td align="center" valign="center" colspan="1" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "N° de Pedido").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Cantidad").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA; border-color: solid white; border-radius: 2px"><b>'.Yii::t("default", "Articulo").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Tipo de Pago").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Referencia").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Banco").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "SubTotal BsS.").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA; color: #DF0101;"><b>'.Yii::t("default", " Deduccion del Servicio BsS.").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA; color: #DF0101;"><b>'.Yii::t("default", "Total a Transferir BsS.").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Total en $").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Estatus").'</b></td>',
	                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Plataforma").'</b></td></tr>');    
...
/***********************************************************************************************/
				/*	foreach ($res as $val) {                                
	                                ...
	                                $data[]=array('<tr align="center" valign="center"><td align="center" valign="center" style="text-align: center;">'.$val['order_id'].'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.$val['qty_item'].'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.$item.'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.strtoupper(Yii::t("default",$val['payment_type'])).'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;"></td>',
	                                      '<td align="center" valign="center" style="text-align: center;"></td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.($val['sub_total'])+($val['packaging']).'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.(($val['sub_total'])*($val['percent_commision']/100)).'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.(($val['sub_total'])+($val['packaging'])) - (($val['sub_total'])*($val['percent_commision']/100)).'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;"></td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.ucwords($val['status']).'</td>',
	                                      '<td align="center" valign="center" style="text-align: center;">'.ucwords($val['request_from']).'</td>');
	                            }                        
	                        }        
*/
	          ...
	          /******************* Reporte de Ventas Diarias por Establecimientos **************************/
						...
						*/
	                    ...
	                    # Consultando la tasa USD del dia
	                    $qryTasaDelDia = "SELECT price FROM {{packages}} WHERE package_id=6";
	                    $tasaDelDia = $db_ext->rst($qryTasaDelDia);
	                    $tasaDelDia = $tasaDelDia[0]["price"];

						# Agregando los datos principales del Reporte de Ventas Diarias por Establecimientos
	                    if ($res=$db_ext->rst($stmt)){
	                    	# Inicializando valor de Totales de las columnas
	                    	$total_subtotal = 0;
	                    	$total_deduccion = 0;
	                    	$total_total_transf = 0;
	                    	$total_en_dolares = 0;
	                        ...
	                        foreach ($res as $val) {                                
	                            ...
	                            # Acumulando Totales de las columnas
		                    	$total_subtotal += $subtotal;
		                    	$total_deduccion += $deduccion;
		                    	$total_total_transf += $total_transf;

	                    		# Consultando la Referencia y el Banco del pedido
	                    		$orderId = $val['order_id'];
	                    		$qryRefBanco = "SELECT payment_reference, payment_provider
	                    						FROM {{payment_order}}
	                    						WHERE order_id='$orderId'";
	                    		$refBanco = $db_ext->rst($qryRefBanco);
	                    		$referencia = $refBanco[0]["payment_reference"];
	                    		$banco = $refBanco[0]["payment_provider"];

		                    	# Agregando valores de las filas de datos
	                            $data[]=array('<tr align="center" valign="center" width="90%" style="border:solid 1px; border-radius:"1px" border-color:solid black; margin-top: 4px;><td align="center" valign="center" style="text-align: center;">'.$val['order_id'].'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.$val['qty_item'].'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.$item.'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.strtoupper(Yii::t("default",$val['payment_type'])).'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.$referencia.'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.$banco.'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.number_format($subtotal, 2, ',', '.').'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.number_format($deduccion, 2, ',', '.').'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.number_format($total_transf, 2, ',', '.').'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.number_format($total_transf/$tasaDelDia, 2, ',', '.').'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.ucwords($val['status']).'</td>',
	                                    '<td align="center" valign="center" style="text-align: center;">'.ucwords($val['request_from']).'</td></tr>');
	                        }

	                        # Agregando el logo ADR 
		    				$head='<div class="uk -form -row" style="margin-bottom: 20px; margin-left: 20px">';
		    				$head.='<table width="25%" align="left" style="padding-left: 40%">
										<tr align="left" valign="center" style="margin-top: 2%; margin-bottom: 2%; height: 20px;">
											<td colspan="2">
							...
							# Agregando el titulo: Informe De Ventas
							$head.='<tr align="left" valign="center" style="margin-top: 20%; border:solid 1px; border-color: solid black; width: 2px;">
										<td align="left"  colspan="2" valign="center" style="text-align: center; font-size: 18; font-style: italic; font-style: bold; max-width: 50%; border-color: solid white;"><b>'.Yii::t("default", "INFORME DE VENTAS").'</b></td>
									</tr>';

		                    # Agregando titulo y datos del Nombre del Establecimiento
		                    $head.='<tr class="table table-bordered" align="left" valign="center" style="background-color: #58ACFA; border: solid 1px; border-radius: 2px; border-color: balck;">
									<td align="center" valign="center" style="text-align: center;"><b>'.Yii::t("default", "Nombre del Establecimiento").'</b></td>
									<td align="center" valign="center" style="text-align: center;">'.$name.'</td>
		                    ...
		                    # Agregando titulo y datos de la Fecha de Cierre
							$head.='<tr class="table table-bordered" align="left" valign="center" style="margin-top: 40px; border: solid 1px; border-radius: 2px; background-color: #58ACFA; border-color: balck;">
									<td align="center" valign="center" style="text-align: center; color: black;"><b>'.Yii::t("default", "Fecha de Cierre").'</b></td>
									<td align="center" valign="center" style="text-align: center; color: black;"><b>'.$date.'</b></td>
										</tr>';
							...
							# Agregando fila de titulos para la tabla de datos
		                    $header=array($head,
		                        '<tr align="center" valign="center" style="argin-top: 40px; border: solid 1px; border-radius: 2px; border-color: balck;"><td align="center" valign="center" colspan="1" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "N° de Pedido").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Cantidad").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA; border-color: solid white; border-radius: 2px"><b>'.Yii::t("default", "Articulo").'</b></td>',
		                        ...
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Referencia").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Banco").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "SubTotal en BsS.").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA; color: #DF0101;"><b>'.Yii::t("default", " Deduccion del Servicio en BsS.").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA; color: #DF0101;"><b>'.Yii::t("default", "Total a Transferir BsS.").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Total en $").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Estatus").'</b></td>',
		                        '<td align="center" valign="center" style="text-align: center; font-style: bold; background-color: #58ACFA;"><b>'.Yii::t("default", "Plataforma").'</b></td></tr>');

	                        # Agregando fila de Totales de las columnas
	                        $data[]=array('<tr align="center" valign="center" width="90%" style="font-weight: bold; border:solid 1px; border-radius:"1px" border-color:solid black; margin-top: 4px;"><td style="border:none"></td>','<td style="border:none"></td>','<td style="border:none"></td>',
	                        	'<td bgcolor="#A9D08E" colspan="3">TOTAL</td>',
	                        	'<td bgcolor="#A9D08E" align="center" valign="center" style="text-align: center;">'.number_format($total_subtotal, 2, ',', '.').'</td>',
	                        	'<td bgcolor="#A9D08E" align="center" valign="center" style="text-align: center;">'.number_format($total_deduccion, 2, ',', '.').'</td>',
	                        	'<td bgcolor="#A9D08E" align="center" valign="center" style="text-align: center;">'.number_format($total_total_transf, 2, ',', '.').'</td>',
	                        	'<td bgcolor="#A9D08E" align="center" valign="center" style="text-align: center;">'.number_format($total_total_transf/$tasaDelDia, 2, ',', '.').'</td>',
	                        	'<td style="border:none"></td>',
	                    		'<td style="border:none"></td></tr>');
	                    }

	                    # Agregando fila de titulo y valor de la Tasa del Dia
	                    $data[]=array('<tr></tr>',
	                    	'<tr align="center" valign="center" width="90%" style="font-weight: bold; border:solid 1px; border-radius:"1px" border-color:solid black; margin-top: 4px;">',
	                    	'<td bgcolor="#FFFF00">TASA DE CAMBIO</td></tr>',
	                    	'<tr align="center" valign="center" width="90%" style="font-weight: bold; border:solid 1px; border-radius:"1px" border-color:solid black; margin-top: 4px;"><td>'.'Bs.S '.number_format($tasaDelDia, 2, ',', '.').'</td></tr>');
		...
		{		
	    	...
	    	if ( $res=$DbExt->rst($stmt)){
	    		foreach ($res as $val) {	
	    			...
	    			//otro eureka
	    			$totalAdminADR=($val['sub_total']*0.16)+$val['sub_total'];
	    			$totalconImpuesto=$val['sub_total']*0.16;
	    			...
	    			$feed_data['aaData'][]=array(
                      ...
                      PrettyFormat($val['total_w_tax']+($val['sub_total']*0.05)+($val['packaging_fee']),$merchant_id),
                      //PrettyFormat((($val['sub_total']*0.05)*2)+($val['delivery_charge']),$merchant_id),
                      //PrettyFormat(($val['total_w_tax'])-($val['sub_total']*0.05)-($val['delivery_charge']),$merchant_id),
	    	...
	    	//reporte de ventas comerciante
	    			...
	    			$feed_data['aaData'][]=array(
                                    $val['order_id'],
                                      ...
                                      '',
                                      '',
                                      PrettyFormat(($val['sub_total'])+($val['packaging'])),
                                      PrettyFormat((($val['sub_total'])*($val['percent_commision']/100))),
                                      PrettyFormat((($val['sub_total'])+($val['packaging'])) - (($val['sub_total'])*($val['percent_commision']/100))),
                                      '',
                                      ucwords($val['status']),
