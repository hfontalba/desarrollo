...
class FunctionsV3
{
    ...
    public static function getMenu($class="menu")
    {
    	...
    	/*$top_menu[]=array('tag'=>"signup",'label'=>''.Yii::t("default","Inicio"),
                'url'=> websiteUrl() );*/
                                              
        $top_menu[]=array('tag'=>"browse",
                'visible'=>in_array('browse',(array)$top_menu_activated)?true:false,
                'label'=>''.Yii::t("default","Descargar App"),
                'url'=>array('index.php/#descargar'));
       ...
       /*  $top_menu[]=array('tag'=>"contact",
                ...
                'label'=>''.Yii::t("default","Contacto"),
                'url'=>array('/store/contact'));                             
                                       
      $top_menu[]=array('visible'=>$client_signup,
                'tag'=>"signup",'label'=>''.Yii::t("default","Ingresar"),
    ...
    public static function getFreeDeliveryTag($merchant_id='')
    {
    	...
    	if ($fee>0){
    		return '<span class="label label-default">'. t("Delivery gratis en ordenes mayores a")." ". self::prettyPrice($fee).'</span>';
    	}
    	return '&nbsp;';
    }
...
/*	public static function getItemFirstPrice($prices='',$discount='')
    ...
    public static function PaymentOptionList()
    {
    	...
    	*/
    	...
    	return array(
    	  'cod'=>t("Transferencia Bancaria"),
    	  'ocr'=>t("Offline Credit Card Payment"),
    	  'pyr'=>t("Mis Pagos Mercantil"),
    	  'pyp'=>t("Paypal"),
    	  'stp'=>t("Stripe"),
    	  'mcd'=>t("Mercapado"),
    	  'ide'=>t("Sisow"),
    	  ...
    	  'bcy'=>t("Barclay"),
    	  'epy'=>t("EpayBg"),
    	  'atz'=>t("Mis Pagos Mercantil (Próximanente)"),
    	  'obd'=>t("Offline Bank Deposit"),
    	  'btr' =>t("mPnadco (Próximamente)"),
    	  'rzr'=>t("Razorpay"),
    	  'vog'=>t("voguepay"),    	  
    	  'trz'=>t("Transferencia Zelle"),
    	  'usc'=>t("Efectivo (USD)"),
    	  'mpt'=>t("Pago Movil"),
    	);
    }
	...
	public static function getMerchantPaymentListNew($merchant_id='')
	{
		...
		switch ($merchant_type) {			
			...
			default:
				...
				if ( getOption($merchant_id,'merchant_disabled_trz')==""){
					$payment_available[]='trz';
				}
				if ( getOption($merchant_id,'merchant_disabled_usc')==""){
					$payment_available[]='usc';
				}
				if ( getOption($merchant_id,'merchant_disabled_mpt')==""){
					$payment_available[]='mpt';
				}
				if ( getOption($merchant_id,'merchant_disabled_ccr')==""){
					$payment_available[]='ocr';
				}
...
// 	public static function sortByOpenCloseTag($a, $b)
// 	{
// 	    $a = $a['tag_raw'];
// 	    $b = $b['tag_raw'];
...
// 	}
    ...
    public static function getPackagesForDeliveryCharges()
	{
		$db_ext=new DbExt;
		$stmt="SELECT title, price FROM
		{{packages}}
		WHERE
		title LIKE 'CobroDelivery_%'
		AND status = 'publish'
		ORDER BY price ASC
		";
		if ($res=$db_ext->rst($stmt)){
	...
	public static function getPackageForUSDAmount()
	{
		$db_ext=new DbExt;
		$stmt="SELECT title, price FROM
		{{packages}}
	...
	public static function getDeliveryChargesByPackage($distance){

		$amount = -1;
		$packs = self::getPackagesForDeliveryCharges();
		if($packs){
			if (is_array($packs) && count($packs)>=1){
				foreach ($packs as $pack) {
					$str_arr = explode ("_", $pack['title']);
					if($distance >= (int)$str_arr[1] && $distance < (int)$str_arr[2]){
						$amount = $pack['price'];
					}
				}
			}
	...
	public static function getTotalAmountUSDByPackage($total){

		$amount = 0;
		$pack = self::getPackageForUSDAmount();
		if (is_array($pack) && count($pack)>=1){
		    $amount = (double)$total/(double)$pack['price'];
		}
