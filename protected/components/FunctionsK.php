...
{	
   ...
   public function getSMSPackagesById($package_id='')
    ...
    /*NUEVO*/
    public function CheckIdentificationCard($id_card='',$client_id='')
    {
    	$and="";
    	if (!empty($client_id)){
    		$and=" AND client_id <> ".q($client_id)." ";
    	...
    	{{client}}
    	WHERE    	
    	id_card LIKE ".FunctionsV3::q("%$id_card")."
    	$and
    	";    	
    	if ( $res=$this->rst($stmt)){      		
    		return true;
    ...
    public function CheckIdentificationCardLoaded($id_card='',$client_id='')
    {
    	$and="";
    	if (!empty($client_id)){
    		$and=" AND client_id = ".q($client_id)." ";
